...
class PriorityQueue(object):
    ...
    def get_task_priority(self, task):
        # Return the Priority of the topmost element in the heap
        if task in self.entry_finder:
    ...
    def remove_task(self, task):
        # Flagging the Task as Removed once we remove it to avoid popping from an empty PQ
    ...
    def peek_task(self):
        while self.pq:
            # Popping the most priority element from the Heap
            priority, removed, task = tuple(heappop(self.pq))
            # And then pushing it back to the PQ
            if removed is False:
...
if __name__ == '__main__':
    ...
    print(min_pq)
    print('Peeking Task',min_pq.peek_task())
